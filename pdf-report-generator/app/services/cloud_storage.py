...
class CloudStorageService:
    ...
    def _get_minio_client(cls):
        ...
        if cls._minio_client is None:
            if not BOTO3_AVAILABLE:
                raise ImportError("boto3 is required for MinIO storage. Install with: pip install boto3")
    ...
    @classmethod
    async def _save_minio(cls, file_data: bytes, filename: str, content_type: str) -> str:
        """Save file to MinIO."""
        loop = asyncio.get_event_loop()
