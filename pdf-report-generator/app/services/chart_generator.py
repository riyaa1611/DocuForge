...
class ChartGenerator:
    ...
    def _create_bar_chart(cls, ax, data: dict, colors: list) -> None:
        ...
        if isinstance(values[0], list):
            ...
            for i, (series_values, series_name) in enumerate(zip(values, data.get("series_names", []))):
                positions = [x + i * width for x in range(n_groups)]
                ax.bar(positions, series_values, width, label=series_name, color=colors[i % len(colors)])
            ...
            # Simple bar chart
            bar_colors = colors[:len(values)] if len(values) <= len(colors) else colors * (len(values) // len(colors) + 1)
    ...
    def _create_line_chart(cls, ax, data: dict, colors: list) -> None:
        ...
        if x and isinstance(x[0], (str, datetime)):
            try:
                x = [datetime.fromisoformat(str(d)) if isinstance(d, str) else d for d in x]
        ...
        if isinstance(y_series[0], list):
            ...
            for i, (y, name) in enumerate(zip(y_series, series_names)):
                ax.plot(x, y, label=name, color=colors[i % len(colors)], linewidth=2, marker='o', markersize=4)
            ax.legend()
        else:
            # Single series
    ...
    def _create_area_chart(cls, ax, data: dict, colors: list) -> None:
        ...
        if isinstance(y_series[0], list):
            for i, (y, name) in enumerate(zip(y_series, series_names)):
                ax.fill_between(x, y, alpha=0.4, color=colors[i % len(colors)], label=name)
    ...
    def _create_scatter_chart(cls, ax, data: dict, colors: list) -> None:
        ...
        ax.scatter(x, y, s=sizes, c=colors[0], alpha=0.6, edgecolors='white', linewidths=1)
        ax.grid(True, alpha=0.3)
